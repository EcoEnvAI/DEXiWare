...
function prepareEvaluation(model,data){
  ...
  const modelPathManager = require(payload.model_path.replace(".dxi",""));
  const tempInputFileName = __tempdir + (new Date()).getTime() + "_evaluation.json";
...
function preparePartialEvaluation(model,data){
  ...
  const modelPathManager = require(payload.model_path.replace(".dxi",""));
  const tempInputFileName = __tempdir + (new Date()).getTime() + "_partial_evaluation.json";
  const tempOutputFileName = tempInputFileName.replace('.json','_output.json');
...
function prepareListInput(operation,model){
  ...
  const modelPathManager = require(payload.model_path.replace(".dxi",""));
  const tempInputFileName = __tempdir + (new Date()).getTime() + "_" + operation + ".json";
  const tempOutputFileName = tempInputFileName.replace('.json','_output.json');
